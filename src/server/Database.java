...
public class Database {
	...
	Database() {
		...
		path = Server.class.getProtectionDomain().getCodeSource().getLocation().getPath();
		try
		{
	...
	private Connection rentConnection()
	{
		Connection ret = null;
		Connection con = null;
		synchronized(idleConnections)
		{
			Iterator<Connection> i = idleConnections.iterator();
			while(i.hasNext())
			{
				con = i.next();
				try
		...
		if(ret == null)
		{
			try
			{
				ret = DriverManager.getConnection("jdbc:sqlite:" + path);
	...
	public void insertAlert(String title, String link, String description, String suggestions, String reasoning) {
		...
		Connection conn = rentConnection();
		try {
